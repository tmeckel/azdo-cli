...
func (t *ttyTablePrinter) AddTimeField(now, tm time.Time, c func(string) string) {
	tf := text.FuzzyAgo(now, tm)
...
var _ TablePrinter = &tsvTablePrinter{}

func (t *tsvTablePrinter) AddTimeField(now, tm time.Time, c func(string) string) {
	tf := tm.Format(time.RFC3339)
	t.AddField(tf, WithColor(c))
}
