...
func (t *Template) Parse(tmpl string) error {
	...
	templateFuncs := map[string]any{
		...
		"join":      joinFunc,
		"markdown": func(input string) (string, error) {
			return markdown.Render(input,
				markdown.WithTheme(t.theme),
...
func (t *Template) ExecuteData(v any) error {
	return t.tmpl.Execute(t.output, v)
}
...
func boldFunc(input any) (string, error) {
	return colorFunc("default+b", input)
}

func colorFunc(colorName string, input any) (string, error) {
...
func pluckFunc(field string, input []any) []any {
	var results []any
...
func timeParse(t, format string) (time.Time, error) {
	return time.Parse(format, t)
}

func timeAgoFunc(now time.Time, t time.Time) (string, error) {
...
func autoColorFunc(colorName string, input any) (string, error) {
	return jsonScalarToString(input)
}

func tableRowFunc(tp tableprinter.TablePrinter, fields ...any) (string, error) {
	if tp == nil {
