	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantsError == "" {
				...
				require.EqualError(t, err, tt.wantsError)
				return
			}
...
func Test_exportFormat_Write(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			exporter: jsonExporter{fields: []string{"s1", "s2"}},
			args: args{
				data: map[string]any{
					"s1": []exportableItem{{"i1"}, {"i2"}},
					"s2": []exportableItem{{"i3"}},
					"s3": []exportableItem{{"i4"}},
				},
			},
			wantW:   "{\"s1\":[{\"s1\":\"i1:s1\",\"s2\":\"i1:s2\"},{\"s1\":\"i2:s1\",\"s2\":\"i2:s2\"}],\"s2\":[{\"s1\":\"i3:s1\",\"s2\":\"i3:s2\"}]}\n",
			wantErr: false,
			istty:   false,
		},
		{
		...
		{
			name:     "export Struct",
			exporter: jsonExporter{fields: []string{"Field1", "Field3"}},
			args: args{
				data: struct {
					Field1 string
					Field2 string
...
func TestStructExportData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out := structExportData(reflect.ValueOf(tt.export), tt.fields, false)
