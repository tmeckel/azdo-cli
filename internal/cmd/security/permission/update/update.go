...
func NewCmd(ctx util.CmdContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update [ORG:][/]SUBJECT | [ORG:]PROJECT/SUBJECT",
		...
		Long: heredoc.Doc(`
			...
			Accepted TARGET formats:
			  - /SUBJECT                       → target subject in the default organization
			  - ORG:/SUBJECT                   → target subject in an explicit organization
			  - PROJECT/SUBJECT                → subject scoped to the project
			  - ORG:PROJECT/SUBJECT            → subject scoped to the project in an explicit organization
		...
		Example: heredoc.Doc(`
			...
			azdo security permission update org:/contoso@example.com --namespace-id 71356614-aad7-4757-8f2c-0fb3bff6f680 --token '$/696416ee-f7ff-4ee3-934a-979b00dce74f' --allow-bit Read

			# Allow multiple actions by specifying --allow-bit multiple times (textual and numeric)
			azdo security permission update org:/contoso@example.com --namespace-id bf7bfa03-b2b7-47db-8113-fa2e002cc5b1 --token vstfs:///Classification/Node/18c76992-93fa-4eb2-aac0-0abc0be212d6 --allow-bit Read --allow-bit Contribute --allow-bit 0x4

			# Allow multiple actions using a single comma-separated value (shells may need quoting)
			azdo security permission update org:/contoso@example.com --namespace-id 302acaca-b667-436d-a946-87133492041c --token BuildPrivileges --allow-bit "Read,Contribute,0x4"

			# Deny a numeric bit and merge with existing ACEs (merge will OR incoming bits with existing ACE)
			azdo security permission update org:/contoso@example.com --namespace-id 33344d9c-fc72-4d6f-aba5-fa317101a7e9 --token '696416ee-f7ff-4ee3-934a-979b00dce74f/237' --deny-bit 8 --merge

			# Use --yes to skip confirmation prompts
			azdo security permission update org:/contoso@example.com --namespace-id 8adf73b7-389a-4276-b638-fe1653f7efc7 --token '$/f6ad111f-42cb-4e2d-b22a-cd0bd6f5aebd/00000000-0000-0000-0000-000000000000' --allow-bit Read --yes
		`),
		Aliases: []string{
			"create",
			"u",
