...
// DescribeBitmaskArray converts a pointer to a permission bitmask into an array of action names.
// When mask is nil or zero, an empty slice is returned.
func DescribeBitmaskArray(actions []security.ActionDefinition, mask *int) *[]string {
	...
	if len(actions) == 0 {
		// No action metadata; return the raw hex as a single-entry string.
		s := fmt.Sprintf("0x%X", *mask)
		return &[]string{s}
	}
	names := make([]string, 0)
	var matched int
