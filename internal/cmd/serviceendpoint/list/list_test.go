...
func (d *deps) expectList(t *testing.T, org, project string) {
	client := mocks.NewMockServiceEndpointClient(d.ctrl)
	d.fact.EXPECT().ServiceEndpoint(gomock.Any(), org).Return(client, nil).AnyTimes()
	client.EXPECT().GetServiceEndpoints(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, args serviceendpoint.GetServiceEndpointsArgs) (*[]serviceendpoint.ServiceEndpoint, error) {
...
func TestRunList_DefaultOrgProjectScope(t *testing.T) {
	t.Parallel()

	d := setup(t)
	d.setupDefaultOrg("myorg")
	d.expectList(t, "myorg", "MyProject")

	err := execute(t, d, "MyProject", "--json")
	require.NoError(t, err)
	assert.Contains(t, d.out.String(), `"name":"MyConnection"`)
}

func TestRunList_ExplicitOrgProjectScope(t *testing.T) {
	t.Parallel()

	d := setup(t)
	d.expectList(t, "otherorg", "MyProject")

	err := execute(t, d, "otherorg:MyProject", "--json")
	require.NoError(t, err)
	assert.Contains(t, d.out.String(), `"name":"MyConnection"`)
}
...
func TestRunList_LegacySlashErrorAndReinterpretation(t *testing.T) {
	...
	d2.expectList(t, "myorg", "otherorg")

	err = execute(t, d2, "otherorg", "--json")
	require.NoError(t, err)
...
func TestNewCmd_UseDocumentsCanonicalSyntax(t *testing.T) {
	...
	cmd := NewCmd(nil)
	require.True(t, strings.HasPrefix(cmd.Use, "list [ORG:]PROJECT"))
	assert.Contains(t, cmd.Example, "myorg:MyProject")
}
