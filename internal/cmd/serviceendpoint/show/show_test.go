...
func TestRunShow_DefaultOrgProjectFirstTarget(t *testing.T) {
	t.Parallel()
	...
	err := execute(t, d, "MyProject/MyConnection", "--json")
	require.NoError(t, err)
	assert.Contains(t, d.out.String(), `"name":"MyConnection"`)
}

func TestRunShow_ExplicitOrgTarget(t *testing.T) {
	t.Parallel()

	d := setup(t)
	d.expectEndpoint(t, "otherorg", "MyProject", "MyConnection")

	err := execute(t, d, "otherorg:MyProject/MyConnection", "--json")
	require.NoError(t, err)
	assert.Contains(t, d.out.String(), `"name":"MyConnection"`)
}

func TestRunShow_LegacySlashErrorAndReinterpretation(t *testing.T) {
	t.Parallel()
	...
	err = execute(t, d2, "otherorg/MyConnection", "--json")
	require.NoError(t, err)
...
func TestNewCmd_UseDocumentsCanonicalSyntax(t *testing.T) {
	...
	cmd := NewCmd(nil)
	require.True(t, strings.HasPrefix(cmd.Use, "show [ORG:]PROJECT/ID_OR_NAME"))
	assert.Contains(t, cmd.Example, "myorg:MyProject/MyConnection")
}
